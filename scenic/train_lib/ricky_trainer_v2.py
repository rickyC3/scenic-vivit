...
def train(
  ...
  if (config.get('quantization', False) and config.quantization.QT_type == "QAT"):
    qt_rules = [
      qwix.QuantizationRule(
          module_path=r"SpatialTransformer/encoderblock_.*/MultiHeadDotProductAttention.*",
          weight_qtype='int8',
          act_qtype='int8',
      )
    ]

    qt_model = qwix.quantize_model(model, qwix.QtProvider(qt_rules))
    model = qt_model

  # -------------- End of QAT rule setting ----------------
  ...
  logging.info('Using gradient accumulation: every_k=%d', every_k_schedule)
  logging.info(tx)
